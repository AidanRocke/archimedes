...
let mc_pi n =
  ...
  let norm = fun x -> List.map Zahlen.square x in
  ...
  let gen size = List.init size (fun _ -> Random.float 1.0) in
...
let newton = fun n -> (Zahlen.da_powah (n+1) *. (Zahlen.factorial n |> float_of_int |> Zahlen.square))
                      /. (Zahlen.factorial (2*n+1) |> float_of_int);;
...
let phi = fun n -> (Zahlen.factorial (4*n) |> float_of_int)
                  /. (Zahlen.factorial n |> float_of_int)**4.;;
...
let rec sum_forms n =
  ...
  else (modular_form n) +. sum_forms (n - 1);;

let ramanujan = fun n -> (1. /. tau) *. (1. /. (sum_forms n));;
